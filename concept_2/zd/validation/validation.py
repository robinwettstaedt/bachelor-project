...
def on_receive_message(ch, method, properties, body):
    # whenever a message is received on the zd-validation queue, retrieve the unvalidated rows from the 'Log' table
    ...
    # and is supposed to trigger the sending of data
        ...
        ch.basic_publish(exchange='', routing_key='zd-validation', body=message)
        print(f"{len(log_data)} unvalidated rows sent back to the validator service via the zd-validation queue. \n")
