...
No data:
    - triggers it to fetch and send the unvalidated rows in its 'Log' table of the EPLF DB back to the validator

Data:
    - contains that compared and therefore validated rows which are then updated in the 'Log' table of the EPLF DB
"""
...
def main():
    ...
    channel.queue_declare(queue='eplf-validation')

    # Declare the callback function
    channel.basic_consume(queue='eplf-validation', on_message_callback=on_receive_message, auto_ack=False)
